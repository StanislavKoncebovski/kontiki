...
namespace Kontiki.Tools.Xml
{
	public static partial class XmlTools
	{
		...
		public static T GetElementValue<T>(this XElement x, string elementName, T defaultValue = default(T))
		{
			Type type = typeof(T);
			object result;
			...
			switch (type)
			{
				...
				case Type _ when type.BaseType == typeof(Enum):
					return (T)GetElementEnum(x, type, elementName, default(T));
		...
		public static T? GetElementValueNullable<T>(this XElement x, string elementName)  where T : struct
		{
			Type type = typeof(T);
		...
		public static T GetAttributeValue<T>(this XElement x, string attributeName, T defaultValue = default(T))
		{
			Type type = typeof(T);
			object result;
			...
			switch (type)
			{
				...
				case Type _ when type.BaseType == typeof(Enum):
					return (T)GetAttributeEnum(x, type, attributeName, default(T));
		...
		public static T? GetAttributeValueNullable<T>(this XElement x, string attributeName)  where T : struct
		{
			Type type = typeof(T);
